...
graph.ssm<-function(N.est,N.yr,weights,years,max.yr, min.yr){
  ...
  plot(0,0,ylim=c(m1,m2),xlim=c(0.5,N.yr),ylab="log(small Cop.)",xlab="year",las=1,col="black",type="l",lwd=2,frame=FALSE,axes=FALSE)
  axis(2,las=1)
...
library(devEMF)
emf("small.Cop.SS.graph.emf",width = 14, height = 7,emfPlus = FALSE)
...
Cop.small.SS.estimates<-cbind(Year,COP_SS_MAP,COP_SS_lower95,COP_SS_upper95)
write.csv(Cop.small.SS.estimates, file = "small.cop.SS.estimates.csv")
