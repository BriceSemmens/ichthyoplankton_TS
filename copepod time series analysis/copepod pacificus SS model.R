...
# Get the scaled copepod data
weights<-(R$Calanus.pacificus - mean(R$Calanus.pacificus))/sd(R$Calanus.pacificus)
...
graph.ssm<-function(N.est,N.yr,weights,years,max.yr, min.yr){
  ...
  plot(0,0,ylim=c(m1,m2),xlim=c(0.5,N.yr),ylab="scaled(Calinus.pacificus)",xlab="year",las=1,col="black",type="l",lwd=2,frame=FALSE,axes=FALSE)
  axis(2,las=1)
...
graph.ssm(N.est,N.yr,weights,years,Y.max)

#Print figure to file
library(devEMF)
emf("Calanus.pacificus.SS.graph.emf",width = 14, height = 7,emfPlus = FALSE)
...
Calanus.pacificus.SS.estimates<-cbind(Year,COP_SS_MAP,COP_SS_lower95,COP_SS_upper95)
write.csv(Calanus.pacificus.SS.estimates, file = "Calanus.pacificus.SS.estimates.csv")
