...
R <- read.csv("Anchovy bulk 15N 20210609.csv", header = T)
...
# pull the bulk data for SS analysis
weights<-R$d15N 
...
N.est[1] ~ dunif(10, 20)            # Prior for initial year
...
graph.ssm.FCL<-function(N.est,N.yr,weights,years,max.yr, min.yr){
  ...
  plot(0,0,ylim=c(m1,m2),xlim=c(min.yr,max.yr),ylab="d15N",xlab="year",las=1,col="black",type="l",lwd=2,frame=FALSE,axes=FALSE)
  axis(2,las=1)
...
library(devEMF)
emf("d15N_StateSpace.emf",width = 14, height = 7,emfPlus = FALSE)
graph.ssm.FCL(N.est,N.yr,weights,years,max.yr,min.yr)
dev.off()
...
d15N_SS_MAP<- fitted
d15N_SS_lower95<- lower95
d15N_SS_upper95<- upper95

d15N.SS.estimates<-cbind(Year,d15N_SS_MAP,d15N_SS_lower95,d15N_SS_upper95)
write.csv(d15N.SS.estimates, file = "d15N.SS.estimates.csv")
